        ...
        with autocast(
            ...
            for _ in range(0, self.num_train_epoch):
                for batch in self.dataloader:
                    ...
                    depth = batch["depth"].to(device=self.device, dtype=self.dtype)
                    first_img = batch["first_image"].to(
                        device=self.device, dtype=self.dtype
                    )
                    first_depth = batch["first_depth"].to(
                        device=self.device, dtype=self.dtype
                    )
                    ...
                    latent = torch.cat([latent_video, latent_depth], dim=2).to(
                        device=self.device, dtype=self.dtype
                    )
                    ...
                    # x0 sample
                    # model_pred = self.scheduler.my_convert_model_output(
                    #     noise_pred, sample=noisy_latent, timestep=timestep
                    # )
