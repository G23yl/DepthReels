...
def get_vae(model_path, device: str | int | torch.device="cuda", weight_dtype=torch.float32) -> WanVAE:
...
def get_transformer(model_path, device: str | int | torch.device="cuda", weight_dtype=torch.bfloat16) -> WanModel:
    cfg = WanModel.load_config(model_path, local_files_only=True)
    logger.info(f"Model config: {json.dumps(cfg, indent=4)}")
    assert isinstance(cfg, dict), "Model config type is not dict"
    transformer = WanModel.from_config(cfg).to(weight_dtype).to(device)
    ...
    for file in os.listdir(model_path):
        if file.endswith(".safetensors"):
            ...
            logger.info(f"Loading model weights from {file_path}")
    ...
    if not found:
        ckpt_name = "model.pt"
        if os.path.exists(os.path.join(model_path, ckpt_name)):
            file_path = os.path.join(model_path, ckpt_name)
            logger.info(f"Loading model weights from {file_path}")
...
def get_text_encoder(model_path, device: str | int | torch.device="cuda", weight_dtype=torch.bfloat16) -> T5EncoderModel:
...
def get_image_encoder(model_path, device: str | int | torch.device="cuda", weight_dtype=torch.bfloat16) -> CLIPModel:
